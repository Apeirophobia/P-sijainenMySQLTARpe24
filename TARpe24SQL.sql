...
as PivotTable
-- põhjuseks, miks näitab NULL-e, et on olemas Id veerg, mida
...
-- A - atomic e aatomlikkus
-- C - consistent e järjepidevus
-- I -  isolated e isoleeritus
-- D - durable e vastupidav

-- Atomic - kõik tehingud transactionis on kas edukalt täidetud v need
...
-- vähendatakse, siis tabelis peab olema vastav kanne. Inventuure ei saa
-- lihtsalt kaduda
...
-- muudetakse. Enamik DB-d kasutab teingute isoleerimise säilitamiseks lukustamist
...
-- käsud ja andmed taastatakse algsesse olekusse. Taastamine stoimub peale
...
-- sama tulemus teha left joinigas
select Name, sum(QuantitySold) as [Total Sales]
from ProductSales
left join Product
...
-- subquerid on alati sulgudes jan neid nimetatase sisemisteks päringuteks
...
select ProductId, Name, Description, UnitPrice, QuantitySold from ProductSales
inner join Product
on ProductSales.ProductId = Product.Id
--valed
...
-- CURSOR
-- relatsiooiliste DB'de haldussüsteemid saavad väga hästi hakkam
-- SETS'ga. SETS lubab mitut päringut kombineerida üheks tuemuseks.
...
-- open avaldsuega täidab select avaldust
...
select distinct Product.Id, Name, Description, UnitPrice from Product
left join ProductSales
on Product.Id = ProductSales.ProductId
where (Name = 'Product - 1000' or Name = 'Product - 65' or Name = 'Product - 1000')
...
иф обжект_ид("эмплойии") ис нулл
бегин
	принт "Тейбл эмплойии крейтед" 
энд
элсе
бегин
	принт "Тейбл эмплойии алреди экзистс" 
энд
...
-- 1. algallika tabel source table
...
-- mis see on?
-- on rühm käske, mis muudavad BD's salvestatud andmeid. Tehingut käsitletakse
-- ühe tööüksusena. Kas kõik käsud õnnestuvad või mitte mingi-
...
-- korraga ühel kasutajal ühe tehingu teha
...
-- muutsin esimese käsuga 9 iphone peale 
-- ikka on 10 tk.
...
-- see juhtub, kui üks transacion loeb samu andmeid kask korda
...
-- uuendasin andmeid, aga esimesest serveris
-- näitab tulemuseks 10 ja peale uuendust ikka 10
-- kuigi uuendasin 5 telefoni pele
truncate table Employee
-- phantom read näide
create table Employee
(
...
-- vastuseks tuleb: Mark ja Sara. Marcust ei nätia, aga peaks

-- erinevus korduvlugemisega ja serialiseerimisega
-- koruduv lugemine hoiab ära ainult kordumataud lugemised
-- serialiseerimine hoiab ära kordumatud lugemised ja phantom read probleemid
-- isolatsioonitase tagab, et ühe tehingu loetud anmded ei 
...
-- samm nr 3
update TableB set Name = 'Mary Transcation 1' where Id = 1
-- samm nr 5
commit transaction
...
-- ohvriks. Seejärel keeratakse deadlockiohvri tehng tagasi ja
-- tagastatakse rakendusele viga 1205. Ohvri tehingu tagasitõmbamine
-- Vabastab kõik selle transactioni valduses olevad lukud.
-- See võimaldab teistel transactionitel blokeeringut tühistada ja edasi liikuda.
...
-- tähtssega valitakse ohvriks
-- 2. kui mõlemal sessioonil on sama prioriteet, siis valitakse
-- ohvriks translaction,
-- mille tagasi viimne on kõige vähem ressurssi nõudev.
-- 3. kui mõlemal seessioonil on sama prioriteet ja sama
-- ressurssi kulutamine, siis ohver valitakse juhuslike alusel
...
-- seak sees hakkab transaction pihta 
-- toimub uuendus tabeliga TableA ja muudab Name
-- selliseks: Mark Transaction 1
-- ootamine 5 sek
-- toimub uuendus tabeliga TableB ja muudab Name
-- selliseks: Mary Transaction 1
...
-- catchi sees omakorda if ja tingimisukes on ERROR_NUMBER() = 1205
